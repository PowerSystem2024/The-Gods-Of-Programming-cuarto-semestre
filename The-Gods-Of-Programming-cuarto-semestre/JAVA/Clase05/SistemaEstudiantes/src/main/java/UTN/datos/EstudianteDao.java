...
//la clase EstudianteDao es el puente entre la aplicación Java y la base de datos MySQL
    ...
    public List<Estudiante> listarEstudiantes() {
        ...
        String sql = "SELECT * FROM estudiantes2025 ORDER BY id_estudiante";
        try {
    ...
    public boolean buscarEstudiantePorId(Estudiante estudiante) {
        ...
        ResultSet rs; //Almacena el resultado obtenido desde la base de datos
        Connection con = getConnection();
        String sql = "SELECT * FROM estudiantes2025 WHERE id_estudiante = ?";
        try {
            ps = con.prepareStatement(sql);
    ...
    public boolean modificarEstudiante(Estudiante estudiante) {
        PreparedStatement ps; //Para preparar la sentencia sql
        Connection con = getConnection();
        String sql = "UPDATE estudiantes2025 SET nombre = ?, apellido = ?, telefono = ?, email = ? WHERE id_estudiante = ?";
        try {
            ps = con.prepareStatement(sql);
            ps.setString(1, estudiante.getNombre());
            ps.setString(2, estudiante.getApellido());
    ...
    public boolean eliminarEstudiante(Estudiante estudiante) {
        ...
        Connection con = getConnection();
        String sql = "DELETE FROM estudiantes2025 WHERE id_estudiante=?";
        try {
            ps = con.prepareStatement(sql);
            ps.setInt(1, estudiante.getIdEstudiante());
            ps.execute();
